...
var swatchseriesschedule = new EvalScript('', {
    run: async function tvschedule(resolv, set) {
        const ewatchopenedvideos = 'openedvideos';
        ...
        if(today) {
             ...
             **/
            let opnd = sc.G.filter(ewatchopenedvideos, el => el.time + 1000 * 60 * 60 * 24 * 6 >= Date.now());
            //new series
            let yesterday = [...days].findIndex(el => el === today) - 1;
            //recently added complete series
            let recentNew = sc.G.filter('recentNewSeries', e => e.ts > Date.now() - 1000 * 60 * 60 * 24 * 14);
            //!!!going minus
