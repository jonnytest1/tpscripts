...
let users: Array<{
    username: string,
    id,
    credentials: Array<{ credId: number, publicKey, prevCounter }>,
}> = []
...
const getOrigin = (userAgent) => {
    ...
    if (userAgent.indexOf('okhttp') === 0) {
        const octArray = process.env.ANDROID_SHA256HASH.split(':')
            .map((h) =>
                parseInt(h, 16),
            );
        const androidHash = base64url.encode(octArray as any);
...
router.post('/registerRequest', csrfCheck, sessionCheck, async (req, res) => {
    ...
    try {
        ...
        const as: any = {}; // authenticatorSelection
