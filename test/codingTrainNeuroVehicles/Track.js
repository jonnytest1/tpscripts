...
function buildTrack() {
    ...
    for (let i = 0; i < total; i++) {
        ...
        let insideX = width / 2 + (xr - pathWidth) * cos(a);
        let insideY = height / 2 + (yr - pathWidth) * sin(a);
        let outsideX = width / 2 + (xr + pathWidth) * cos(a);
        let outsideY = height / 2 + (yr + pathWidth) * sin(a);
        checkpoints.push(new Boundary(insideX, insideY, outsideX, outsideY));
        inside.push(createVector(insideX, insideY));
        outside.push(createVector(outsideX, outsideY));
    }
    walls = [];
