...
function findCreate() {
     ...
     * @type {any}
     */
    let t = function (string, iF, compress) {
        if (!compress) {
            compress = true;
        }
        let obj = t.I(string);
    ...
    t.a = async function get(identification, parent, tag, finder = t) {
        ...
        return new Promise((resolve) => {
            function waitTillDefined(identification) {
                let obj = finder(identification, parent);
                ...
                if (obj) {
                    console.log('found for ' + identification);
                    resolve(obj);
                    return;
                }
                else {
                    //console.log("didnt find for " + identification);
                    setTimeout(waitTillDefined, 500, identification);
                }
            }
